...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        # The probability value corresponding to the number of gene copies and whether a person exhibits the trait
        trait_prob = PROBS["trait"][num_genes][with_trait]
        ...
        if mother is None and father is None:
            ...
            passing_prob = {0: PROBS["mutation"], 1: 0.5, 2: 1 - PROBS["mutation"]}
            ...
            if num_genes == 0:
                ...
                probability *= (1 - passing_prob[mother_gene]) * (1 - passing_prob[father_gene])
            elif num_genes == 1:
                # 1, one of parents gave gene
                probability *= (1 - passing_prob[mother_gene]) * passing_prob[father_gene] + passing_prob[mother_gene] * (1 - passing_prob[father_gene])
            else:
                # 2, both of parents gave gene
                probability *= passing_prob[mother_gene] * passing_prob[father_gene]
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        if trait_sum != 0:
            for with_trait in trait_distribution:
                trait_distribution[with_trait] /= trait_sum
